function [Y_predict] = aditya28_RDA_test(X_test, RDAmodel, numofClass)
%
% Testing for RDA
%
...
[V,W] = size(X_test);
RDA = zeros(V,numofClass);
inv_Sigmapooled = inv(RDAmodel.Sigmapooled);
for i = 1:V
    for j = 1:numofClass
       RDA(i,j) =((RDAmodel.Mu(j,:))*(inv_Sigmapooled))*(X_test(i,:)') - (0.5*RDAmodel.Mu(j,:))*(inv_Sigmapooled)*((RDAmodel.Mu(j,:))') + (log(RDAmodel.Pi(j,:)));
    end
end
[value,Y_predict] = max(RDA, [], 2);
